...
import {
  ...
  AlertTriangle, Clock, Shield, Star, IndianRupee, TrendingUp
} from 'lucide-react'
...
export default function BillingPage() {
  ...
  async function handleUpgrade(planName: string) {
    ...
    try {
      const token = localStorage.getItem('auth_token')
      const res = await fetch(`${API_BASE}/api/organizations/subscription/upgrade/`, {
        method: 'POST',
        headers: {
      ...
      <div className="min-h-screen bg-background flex items-center justify-center">
        <Card className="max-w-md w-full">
          <CardContent className="pt-6 text-center">
            ...
            <p className="text-muted-foreground mt-2">{error || 'Your organization does not have an active subscription.'}</p>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
            ...
            <CardContent className="py-4 flex items-center justify-between flex-wrap gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">Upgrade now to keep using all features without interruption.</p>
                </div>
              </div>
            ...
            <CardContent className="py-4 flex items-center justify-between flex-wrap gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">Your account is in read-only mode. Upgrade to resume adding loans, customers, and collections.</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="p-4 bg-muted/30 rounded-xl">
                  <div className="flex items-center gap-2 text-muted-foreground mb-1">
                    ...
                    <span className="text-sm">{subscription.is_trial ? 'Trial Ends' : 'Renews On'}</span>
                  </div>
                  <p className="text-xl font-bold text-foreground">
          ...
          <Card className="border-border/50">
            ...
            <CardContent className="space-y-5">
              <div>
                <div className="flex items-center justify-between text-sm mb-2">
                  ...
                  <span className="font-medium text-foreground">
                    {subscription.current_users} / {subscription.max_users >= 999 ? '∞' : subscription.max_users}
                  </span>
                </div>
                <div className="w-full bg-muted rounded-full h-2">
                  <div
                    className={`h-full rounded-full transition-all ${
                      subscription.current_users >= subscription.max_users ? 'bg-red-500' : 'bg-primary'
                    }`}
              ...
              <div>
                <div className="flex items-center justify-between text-sm mb-2">
                  <span className="flex items-center gap-2 text-muted-foreground">
                    <UserPlus className="w-4 h-4" /> Customers
                  </span>
                  <span className="font-medium text-foreground">
                    {subscription.current_customers} / {subscription.max_customers >= 9999 ? '∞' : subscription.max_customers}
                  </span>
                </div>
                <div className="w-full bg-muted rounded-full h-2">
                  <div
                    className={`h-full rounded-full transition-all ${
                      subscription.current_customers >= subscription.max_customers ? 'bg-red-500' : 'bg-emerald-500'
                    }`}
        ...
        <div id="plans">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {subscription.available_plans.map((plan) => {
              ...
              return (
                <Card
                  key={plan.name}
                  className={`relative border-border/50 shadow-sm hover:shadow-lg transition-all h-full flex flex-col ${
          ...
          <p className="text-center text-sm text-muted-foreground mt-6">
            💳 Payment integration coming soon. Contact us at <a href="tel:+917418573018" className="text-primary hover:underline">+91 74185 73018</a> for manual activation.
          </p>
        </div>
      </main>
    </div>
